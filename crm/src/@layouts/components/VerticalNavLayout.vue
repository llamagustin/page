...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const showMenuDashboard = ref(true);
    ...
    const belongsToAdministratorList = () => {
      return MenuDashboard.some(item => item?.to == route.path || route.path.startsWith('/apps/form'));
    }
    
    watch(() => route.path, (path) => {
      showMenuDashboard.value = belongsToAdministratorList() && windowWidth.value > 1280
    }, { immediate: true });
    ...
    watch(windowWidth, value => {
      ...
      if(value < 1280 || !belongsToAdministratorList()) {
        showMenuDashboard.value = false
      } else {
        if(!showMenuDashboard.value) {
          showMenuDashboard.value = true
        }
      }
    }, { immediate: true })
    ...
    return () => {
      ...
      const menuDashboard = slots['menu-dashboard'] ? h(
        'div',
        { class: ['layout-navbar-sticky','menu-dashboard', { 'd-none': !showMenuDashboard.value }] },
        [
          h('div', { class: 'layout-navbar px-1'}, slots['menu-dashboard']?.())
        ]
      ) : null;
      ...
      return h(
        ...
        [
          ...
          h(
            'div',
            { class: ['layout-content-wrapper', (slots['menu-dashboard'] ? { 'has-menu-dashboard': showMenuDashboard.value } : {}) ] },
            [
              navbar,
              main,
              footer,
...
.layout-wrapper.layout-nav-type-vertical {
  ...
  .has-menu-dashboard {
    padding-inline-end: var(--menu-dashboard);
