...
function selectProvince(reset=true) {
...
async function selectCity() {
  ...
  if(ubigeo.value.code[0]) {
    dataForm.value.address_zip = ubigeo.value.code[0].codigo_postal
  }
...
<template>
  <div>
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        ...
        <ButtonStep
          type-arrow="right"
          :is-disabled="!isValid"
          @click="isValid ? show(4) : showErrors()"
        />
      </div>
    </div>
    ...
    <VRow class="mb-8">
      ...
      <VCol cols="12" sm="12" md="7" v-if="dataForm.address_province">
        <label class="block text-sm font-medium" for="city">
