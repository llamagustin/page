...
import { formatDate, avatarText } from "@core/utils/formatters";
...
const contact = computed(() => ({
  ...
  name: store.activeChat?.contact.fullName,
...
const msgGroups = computed(() => {
  ...
  if (store.activeChat!.messages) {

    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    messages = store.activeChat!.messages;

    let msgSenderId = store.activeChat.user_id;
...
<template>
  <div class="chat-log pa-5">
    ...
    >
      <div
        v-if="msgGrp.messages?.length"
        class="chat-avatar"
        :class="msgGrp.senderId !== contact.id ? 'ms-4' : 'me-4'"
      >
        <VAvatar size="38" variant="tonal" color="success">
          <span v-if="msgGrp.senderId === contact.id">
            {{ avatarText(contact.name?.name || '?') }}
          </span>
          <span v-else>
            {{ avatarText(store.auth?.fullName || '-') }}
            <!--{{ avatarText(store.auth?.name?.name || '-') }}-->
          </span>
          <!--
          <VImg
            :src="
