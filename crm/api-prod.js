...
app.use('/template/:id', async function(req, res) {
  ...
  try {
    var data = await s3.getObject({ Bucket: "aythen.template", Key: url }).promise();
    //var html = data.Body.toString('utf-8');

    console.log('d', data)

    var base = Buffer.from(`<base href="https://s3.eu-central-1.amazonaws.com/aythen.template/${key}/" >`, 'utf-8');
...
const io = require("socket.io")(server, {
  cors: {
    origin: "https://reseller.aythen.com",
    methods: ["GET", "POST"],
...
io.on('connection', (socket) => {
    console.log("==============================");
    console.log("SOCKETIO CONNECTED 4005");
    console.log("==============================");
...
function confirmSign(req, res){
  console.log('vv', req.body.status)
  if(req.body.status == 'done'){
    io.emit("received-pdf", {
      id: req.body.job
    });  
  }
...
app.post('/unataca/confirm/:id', confirmSign);
