<script setup lang="ts">
  ...
  onMounted(() => {
    for (let tab of props.tabs) {
      if (tab.default && selected.value.length === 0) {
  ...
  <div class="tabs-wrapper" :class="[sliderClass]">
    <div class="tabs-inner">
      <div class="tabs" :class="[
        ...
        <ul>
          <li v-for="(tab, key) in tabs" :key="key" :class="[selected === tab.value && 'is-active']">
            <slot name="tab-link">
              <a @click="selected = tab.value" @keydown.space.prevent="() => (selected = tab.value)">
                <Icon v-if="tab.icon" :icon="tab.icon" />
                <span class="px-1">{{ tab.label ? tab.label : $t('waitTab' + key) }}</span>
              </a>
            </slot>
          </li>
    ...
    <div class="tab-content is-active">
      <TransitionGroup :name="props.slow ? 'fade-slow' : 'fade-fast'" mode="out-in">
        <slot name="tab" :selected="selected"></slot>
      </TransitionGroup>
    </div>
  </div>
</template>
