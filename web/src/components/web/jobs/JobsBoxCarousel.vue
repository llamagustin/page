    ...
    <div class="box-carousel">
      <div class="box-carousel-title max-w-7 mb-6">
        <Subtitle tag="h3" :size="6" weight="bold" class="pb-2">
          <span class="text-gradient">{{$t('freelancer_minititle')}}</span>
        </Subtitle>
        <Title tag="h2" :size="3" weight="bold">
          <span>{{$t('freelancer_title1')}}</span>
        </Title>
        <p class="paragraph rem-115">{{$t('freelancerParagraph')}}</p>
      </div>
      <carousel :items-to-show="9">
        <slide v-for="(slide, index) in props.slides" :key="index">
          <RouterLink :to="'/team/offer'" class="box-carousel-box">
            <div class="box-inner">
      ...
      <!--  <Button :to="'/team/offer'" color="primary" raised>{{$t('freelancerStarted')}}</Button>-->
      <!--</div>-->
    </div>
  </div>
</template>
