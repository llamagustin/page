...
<template>
   <div class="job-wrapper">
    <RouterLink to="/team/freelancer" class="back-link">
      <i class="iconify" aria-hidden="true" data-icon="feather:arrow-left"></i>
      <span class="return-back-colors">Back to Freelancers</span>
    </RouterLink>
    <div class="job-header">
      ...
      <div v-if="newJob.contact" class="right">
        <Button @click="modalLargeOpen = true" class="buttons-styles" :long="2" outlined raised>Editar</Button>
        <Button class="buttons-styles" :long="3" outlined raised>Aplicar</Button>
      </div>
    </div>
    <div class="job-body">
      
      <div class="columns">
        <div class="column is-6 cv">
          
          <div class="rows">
            <div class="row">
              <h3 class="underline letter-center">Sobre Nosotros</h3>
              <p class="letter-center" style="text-align: center;">{{newCategory}}, {{newSubcategory}} :{{newProfile.about}}</p>
            </div>
            <div class="row">
              <h3 class="underline letter-center">Perfil</h3>
              <p class="letter-center" style="text-align: center;">{{newCategory}}, {{newSubcategory}} :{{newProfile.profile}}</p>
            </div>
        
            <div class="row">
              <h3 class="underline letter-center">Estudios</h3>
              <table v-for="exp in newProfile.academies" style="width:100%;" >
                <tr >
                  <td style="width:100px;" > <b> Institution:</b></td>
                  <td> {{exp.institution.toUpperCase()}}</td>
                </tr>
                <tr>
                  <td><b> Grado:</b></td>
                  <td> {{exp.degree.toUpperCase()}}</td>
                </tr>
                <tr>
                  <td></td>
                  <td>
                    <span class="is-size-7" >
                      <b>Start: </b>{{exp.startDate}}      
                      <b>End:</b> {{exp.endDate}}
                    </span>
                  </td>
                </tr>
        ...
        <div class="row experiences">
          <h3 class="underline letter-center">Experiencias</h3>
          <table v-for="exp in newProfile.experiences" >
            <tr >
              <td style="width:100px;" > <b> Company:</b></td>
              <td> {{exp.company.toUpperCase()}}</td>
            </tr>
            <tr>
             <td><b> Position:</b></td>
              <td> {{exp.position.toUpperCase()}}</td>
            </tr>
        ...
        <div class="column is-5 is-offset-1 ">
           <div >
                <Container v-if="nft">
                  <TaskDetails :profile="newProfile" :nft="nft" />
                </Container>
           </div>
        </div>
      </div>
   ...
   <Modal :title="showForm ? 'Edit Freelancer' : 'Edit Crm Freelancer'" :open="modalLargeOpen" size="big" actions="center" noscroll @close="modalLargeOpen = false">
     <template #content>
       <FreelancerForm :profile="newProfile"/>
     </template>
      <template #action v-if="showForm">
        <div class="flex space-between">
          <Button 
            @click="handleSaveFreelancer" 
            color="primary" 
            :long="2" 
            raised
            outlined>
